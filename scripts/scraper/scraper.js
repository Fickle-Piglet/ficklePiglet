...
module.exports = {
    ...
    getTags : function(searchTerm){
        ...
        request({
            ...
            for(var i=0; i<body.results.length; i++){
                ...
                fs.writeFile('../scripts/scraper/files/'+ body.results[i].trackId +'.txt', JSON.stringify(target), function(err){
                    if (err) return console.log(err);
    ...
    readAllFiles : function(){
        fs.readdir("../scripts/scraper/files/", function(err, filenames) {
            if (err) {
                onError(err);
                return;
            }
            filenames.forEach(function(filename) {
                fs.readFileSync("../scripts/scraper/files/" + filename, 'utf-8', function(err, content) {
                    if (err) {
                        onError(err);
                        return;
                    }
                    fs.writeFile('../scripts/scraper/targetFile/results.txt', JSON.stringify(content)+"\n", function(err){
                        if (err) return console.log(err);
                        console.log(">>>>>>>>SUCCESS");
                    })
